(function()
{
    ...
    function styleSheet(set)
    {
        if (set === true)
        {
            ...
            style.innerHTML = '#floatingWindow { z-index: 9445656445656; line-height: 1.2; position: fixed; overflow: hidden; top: 10%; width: 14%; height: 40%;' + 
                              'background: #000; color: #fff; opacity: 0.8; border: 1px solid lightblue; padding: 1%; font-size: 0.75vmax; }';
            // ##########################################################################
            // # ID floatingWindowData (main window's display. Displays element's data) #
            // ##########################################################################

            style.innerHTML += '#floatingWindowData { overflow: hidden; max-height: 79%; min-height: 40px; }';

            // #######################################################
            // # ID floatingWindowOptions (main window options' div) #
            // #######################################################

            style.innerHTML += '#floatingWindowOptions { z-index: 9445656445657; position: absolute; bottom: 20%; width: 100%; height: 5%; background: transparent;' +
            'color: lightblue; opacity: 0.8; padding: 1%; font-size: 0.65vmax; }';

            // #################################################################################
            // # CLASS fWOption (for menu elements, or elements that must be made 'clickable') #
            // #################################################################################

            style.innerHTML += '.fWOption { color: lightblue; cursor: pointer }';
    ...
    var mouseoverEventListener = function(e)
    {
        floatingWindowData.innerHTML =
            "<p><b>Type: </b>" + e.target.tagName + "</p>" + "<p><b>ID: </b>" + e.target.getAttribute("id") + "</p>" + "<p><b>Class: </b>" + e.target.getAttribute("class") + "</p>" + "<p><b>Target: </b>" + e.target + "</p>";
    };
    document.addEventListener("mouseover", mouseoverEventListener);

    var documentClickListener = function(e)
    {
        if (e.target.getAttribute("id") == "clickRemoves")
        {
        ...
        if (e.target.getAttribute("id") == "preventLinks")
        {
        ...
        if (e.target.getAttribute("id") == "minimizeDivRem")
        {
            switch (minimized)
            {
                case false:
        ...
        if (e.target.getAttribute("id") == "enableButtons")
        {
        ...
        if (e.target.getAttribute("id") == "displayHidden")
        {
        ...
        if (e.target.getAttribute("id") == "closeDivRem")
        {
            styleSheet("destroy");
